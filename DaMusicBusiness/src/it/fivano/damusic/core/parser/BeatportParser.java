...
import it.fivano.damusic.conf.BeatportConf;
import it.fivano.damusic.core.parser.model.BeatportParserModel;
import it.fivano.damusic.core.parser.model.GenericParser;
import it.fivano.damusic.core.utils.DamusicUtility;
import it.fivano.damusic.core.utils.DamusicUtility.LevelSimilarity;
...
//			System.out.println("\tBEATPORT_TRACK: "+listTrack.size()+" "+listTrack);
			...
			release.setTracks(DamusicUtility.chooseTrack(release.getTracks(), listTrack, false));
			...
			DamusicUtility.updateReleaseExtraction(release.getReleaseExtraction(),true,AreaExtraction.BEATPORT);

		} catch(Exception e) {
			log.error("Errore nel parsing", e);
			DamusicUtility.updateReleaseExtraction(release.getReleaseExtraction(),false,AreaExtraction.BEATPORT);
//			throw new ParseReleaseException("Errore nel parsing",e);
		}
	...
	private BeatportParserModel popolaBeatport(Element e, String releaseName) {
		...
		double simil = DamusicUtility.getStringSimilarity(releaseNameSearch,titleString,LevelSimilarity.ALTO);
	...
	private ReleaseModel popolaRelease(ReleaseModel release, BeatportParserModel beatportModel) throws ParseException {
		...
		if(release.getNameWithUnderscore()==null && beatportModel.getReleaseName()!=null) {
			...
			DamusicUtility.processReleaseName(release);
		}
