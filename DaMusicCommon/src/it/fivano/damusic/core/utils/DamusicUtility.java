...
import it.fivano.damusic.model.ReleaseExtractionModel;
import it.fivano.damusic.model.ReleaseModel;
import it.fivano.damusic.model.TrackModel;
import it.fivano.damusic.model.ReleaseExtractionModel.AreaExtraction;

public class DamusicUtility {
	...
	public static Properties getProps(String propsName) throws IOException {
		InputStream in = DamusicUtility.class.getClassLoader().getResourceAsStream(propsName);
	...
	public static void processReleaseName(ReleaseModel rel) {
		Map<String,Integer> qualityMap = new HashMap<String, Integer>();
		processReleaseName(rel, qualityMap);
	}

	public static void processReleaseName(ReleaseModel rel, Map<String,Integer> qualityMap) {
...
//		String[] nameSplit = name.replace(" ","_").replace("-","_").split("_");
		...
		if(year!=null && year.matches("\\d\\d\\d\\d") && rel.qualitaDatiMigliore(C.CREW, 5) && rel.qualitaDatiMigliore(C.YEAR, 5) ) {
			rel.setCrew(crew);
			rel.setYear(year);
			rel.getMappaQualitaDati().put(C.CREW, 5);
			rel.getMappaQualitaDati().put(C.YEAR, 5);
		}

		if(nameSplit.length>2 && rel.qualitaDatiMigliore(C.ARTIST, 5) && rel.qualitaDatiMigliore(C.SONG, 5) )  {
			rel.setArtist(nameSplit[0].replace("_", " "));
			rel.setSong(nameSplit[1].replace("_", " "));
			rel.getMappaQualitaDati().put(C.ARTIST, 5);
			rel.getMappaQualitaDati().put(C.SONG, 5);
		}
